    ...
    return [
        {
            ...
            "type": "function",
            "description": "Retrieve current weather conditions (temperature, wind, cloud cover) for the plant location.",
            "parameters": {
                "type": "object",
                "properties": {},
                "required": [],
...
def _run_client_tool(name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
    ...
    """
    if name == "get_weather_stub":
        return agent_tools.get_live_weather_features() or {"error": "Weather data unavailable"}
...
def _check_relevance(user_query: str) -> bool:
    """
    Guardrail: Checks if the user query is relevant to Curtailment, Dispatch, Solar, or Energy.
    Returns True if relevant, False otherwise.
    """
    if client is None:
        return True # specific guardrail: fail open if no LLM
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "You are a strict guardrail for an energy management AI.\n"
                        "Your ONLY job is to determine if the User Query is strictly related to:\n"
                        "- Solar Energy / Renewable Energy\n"
                        "- Battery Storage (BESS)\n"
                        "- Power Dispatch / Curtailment\n"
                        "- Grid Engineering / Economics\n"
                        "- Weather (in the context of generation)\n"
                        "\n"
                        "If the query is about ANYTHING else (e.g. cooking, history, general chat, coding, jokes), return 'FALSE'.\n"
...
def run_agentic_dispatch(user_query: str, plant_meta: Dict[str, Any] = None) -> str:
    ...
    if not _check_relevance(user_query):
        return "User Inquery is irrelevant"
            ...
            input=[
                {
                    ...
                    "content": (
                        ...
                        "  1. Call get_weather_stub (obtain current weather conditions)\n"
                        "  2. Call get_solar_forecast_stub (obtain a 6-interval solar forecast horizon)\n"
                        "  3. Call get_rag_insights_stub with the user query\n"
                        "  4. Call solve_milp_dispatch_stub using ONLY the BESS parameters from the user "
                        "(default to 50 MWh capacity and 35% SoC when unspecified) and the horizon prepared "
                        "by the tools.\n"
                        "Only after all FOUR tool outputs are available may you craft a final answer.\n"
                        "Your final answer MUST follow this exact Markdown structure:\n"
                        "\n"
                        "--- Agent Final Answer ---\n"
                        "\n"
                        "### Current Weather:\n"
                        ...
                        "### Forecast Horizon:\n"
                        "- **Time Interval**: [Date]\n"
                        "- **Forecast Power Output**: [MW value]\n"
                        "- **Confidence**: [Value%]\n"
                        "\n"
                        "| Time Interval | Forecast MW |\n"
                        "|---|---|\n"
                        "| [Time] | [MW] |\n"
                        "... (for all 6 intervals)\n"
                        "\n"
                        "### Insights for Curtailment Reduction:\n"
                        "[Bullet points from RAG insights]\n"
                        "\n"
                        "### MILP Dispatch Recommendations:\n"
                        "- **Current SoC**: [Value]%\n"
                        "- **Planned Dispatch**: [Summary]\n"
                        "- **Resulting SoC**: [Value] MWh\n"
                        "\n"
                        "| Time Interval | Dispatch MW | Charge MW | Discharge MW | Curtailment MW | End SoC (MWh) |\n"
                        "|---|---|---|---|---|---|\n"
                        "| [Time] | [MW] | [MW] | [MW] | [MW] | [MWh] |\n"
                        "... (for all 6 intervals)\n"
                        "\n"
                        "### Engineering Insights:\n"
                        "[Technical analysis of the dispatch plan, grid constraints, and BESS usage]\n"
                        "\n"
                        "### Conclusion:\n"
                        "[Final summary]\n"
                        "\n"
